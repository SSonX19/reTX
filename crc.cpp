...
uint8_t GENERIC_CRC8::calc(const uint8_t data) const
{
    return _crc8tab[data];
}

uint8_t GENERIC_CRC8::calc(const uint8_t *data, uint16_t len, uint8_t crc) const
{
    while (len--)
    {
        crc = _crc8tab[crc ^ *data++];
    }
    return crc;
}
...
uint16_t Crc2Byte::calc(uint8_t *data, uint8_t len, uint16_t crc) const
{
    while (len--)
    {
