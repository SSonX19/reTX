...
typedef enum
{
    ...
    // https://github.com/ArduPilot/ardupilot/blob/a2d5a32b4c14048551452c37b5e260ae6d2451bc/libraries/AP_RCProtocol/AP_RCProtocol_CRSF.h#L78
...
// From: https://github.com/betaflight/betaflight/blob/a35a5af16c917139507c84aa1b9e504a25e060b2/src/main/rx/crsf_protocol.h#L95C1-L103C3
enum {
...
typedef struct crsf_ext_header_fields_s
{
    uint8_t dest_addr;
    uint8_t orig_addr;
} PACKED crsf_ext_header_fields_t;
...
typedef struct crsf_packet_s
{
    ...
    union {
        ...
        // struct {
        //     crsf_ext_header_fields_t ext;
        //     deviceInformationPacket_t device_info;
        //     // No CRC due to dynamic size
        // } PACKED device_info;

        // CRSF_FRAMETYPE_BATTERY_SENSOR
        struct {
            crsf_sensor_battery_t data;
            uint8_t crc;
        } PACKED sensor_battery;

        // CRSF_FRAMETYPE_BARO_ALTITUDE
        struct {
            crsf_sensor_baro_vario_t data;
            uint8_t crc;
        } PACKED sensor_baro_vario;

        // CRSF_FRAMETYPE_GPS
        // Ref: https://github.com/iNavFlight/inav/blob/master/src/main/telemetry/crsf.c
        struct {
            int32_t lat; // Latitude ( degree / 10`000`000 )
            int32_t lon; // Longitude (degree / 10`000`000 )
            uint16_t ground_speed; // Groundspeed ( km/h / 10 )
            uint16_t ground_course; // GPS heading ( degree / 100 )
            uint16_t altitude; // Altitude ( meter ­1000m offset )
            uint8_t num_sat; // Satellites in use ( counter )
            uint8_t crc;
        } PACKED sensor_gps;

        // CRSF_FRAMETYPE_VARIO
        struct {
            int16_t vert_speed; // Vertical speed ( cm/s )
            uint8_t crc;
        } PACKED sensor_vario;
        ...
        struct {
            char name[CRSF_FRAME_SIZE_MAX-3];
        } PACKED flight_mode;

    };
} PACKED crsf_packet_t;

static_assert(sizeof(crsf_packet_t) == 64);
